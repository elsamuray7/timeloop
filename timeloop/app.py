...
class Timeloop():
    ...
    def _add_job(self, func, interval, initial_delay, *args, **kwargs):
        j = Job(interval, func, initial_delay, *args, **kwargs)
        self.jobs.append(j)
    ...
    def job(self, interval, initial_delay=None):
        def decorator(f):
            self._add_job(f, interval, initial_delay)
            return f
        return decorator
